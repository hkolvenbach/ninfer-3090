...
from tools.reference.qwen3_8_27b.bindings import ArtifactBinding
...
def binding():
    path = Path(
        os.environ.get(
            "NINFER_QWEN3_8_27B_ARTIFACT",
            PROJECT_ROOT / "out/qwen3_8_27b.ninfer",
        )
    )
    if not path.is_file():
        pytest.skip(f"real Qwen3.8-27B artifact is not available at {path}")
    with ArtifactBinding.open(path) as value:
        yield value


def test_complete_typed_binding_and_vision_policy(binding: ArtifactBinding) -> None:
    assert binding.identity == ArtifactIdentity("qwen3.8-27b", "groupwise-int")
