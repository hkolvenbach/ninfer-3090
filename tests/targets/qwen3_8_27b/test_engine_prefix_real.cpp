...
int exercise_registered_frontend(const ninfer::Engine& engine, bool qwen3_8) {
    ...
    const std::uint32_t expected_thinking = qwen3_8 ? 58U : 16U;
    const std::uint32_t expected_no_thinking = qwen3_8 ? 18U : 18U;
...
int exercise_artifact(const char* artifact) {
    ...
    if (const int result =
            exercise_registered_frontend(engine, engine.load_summary().target == "qwen3_8_27b");
        result != 0) {
        return result;
    }
...
int main() {
    ...
    const char* nvfp4     = std::getenv("NINFER_QWEN3_8_27B_NVFP4_WEIGHTS");
    if ((groupwise == nullptr || *groupwise == '\0') && (nvfp4 == nullptr || *nvfp4 == '\0')) {
        std::cout << "skip: neither NINFER_QWEN3_8_27B_WEIGHTS nor "
                     "NINFER_QWEN3_8_27B_NVFP4_WEIGHTS is set\n";
        return 77;
    }
