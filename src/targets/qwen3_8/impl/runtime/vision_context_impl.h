...
std::size_t VisionContext::workspace_bytes(const qwen3_8::VisionItemControl& item) {
...
void VisionContext::encode(const VisionItemView& item, Tensor& output,
    ...
    const qwen3_8::VisionItemControl& control = *item.control;
...
VisionPrefillSession::VisionPrefillSession(DeviceContext& device, const LoadedModelData& model,
                                           ...
                                           qwen3_8::PreparedPromptData& prompt,
...
VisionChunk VisionPrefillSession::prepare_chunk(std::uint32_t begin, std::uint32_t nominal_length) {
    ...
    const qwen3_8::VisionItemControl& control = plan_.control->items[active->item_index];
    const qwen3_8::VisionItem& source         = prompt_.vision_items[active->item_index];
