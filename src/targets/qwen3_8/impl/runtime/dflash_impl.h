...
template <class V>
void propose_batch_impl(DFlashBatchContext& state, qwen3_8::DFlashDecodeState& frame,
...
auto dflash_decode_batch_body(DFlashBatchContext& state, std::int32_t batch_size, std::uint32_t k,
    ...
    return [&state, batch_size, k, envelopes, target_envelope] {
        ...
        qwen3_8::DFlashDecodeState& frame = state.frame;
        const std::int32_t width          = static_cast<std::int32_t>(k) + 1;
        ...
        CUDA_CHECK(cudaMemcpyAsync(&state.host_egress, frame.egress.data,
                                   sizeof(qwen3_8::DFlashDecodeEgress), cudaMemcpyDeviceToHost,
                                   state.execution.device.stream));
    };
}
