...
/** Qwen3.8's target-local mapping from a stable request lane to its two state roles. */
struct LinearStateSlots {
    [[nodiscard]] static std::int32_t state_slot_count(std::uint32_t max_concurrency) {
        if (max_concurrency == 0 ||
            ...
            throw std::invalid_argument("Qwen3.8 Linear Attention concurrency is invalid");
        }
    ...
    [[nodiscard]] static std::int32_t current_state_slot(std::uint32_t lane,
        ...
        if (lane >= max_concurrency) {
            throw std::out_of_range("Qwen3.8 Linear Attention lane is out of range");
        }
